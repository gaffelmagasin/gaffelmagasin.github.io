...
function draw_article_button(div_id, article_name,small, use_preamble = true){
	...
	.then(function(resp){
		...
		if(small == true){
			...
			text_style_preamble = `font-size: clamp(10px,4.5vw,35px);`
		...
		new_html +=`<div style = "width: ${roww}%; margin: 3%; float: left;">`
			...
			//titel
			new_html += `<div id = ${'titel' + div_id} style = "word-break: break-word; hyphens: auto; color: white; width: 98%; padding-top: 2%; font-family: legaff; font-size: clamp(10px,4vw,38px); line-height: 100%;${text_style_title}"> ${titel}</div>`
...
function draw_article_button_small(div_id, article_name,small){
	...
	.then(function(resp){
		...
		//start
		new_html +=`<div style = "width: 100%; margin: 3%; float: left;">`
		
			//titel
			new_html += `<div id = ${'titel' + div_id} style = "color: white; width: 98%; padding-top: 2%; font-family: morfeta; font-size: clamp(10px,4vw,38px); line-height: 100%;${text_style_title}"> ${titel}</div>`
